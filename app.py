...
df_stats = pd.DataFrame({'青壯方案(實體)': count_gen, '通訊諮商': count_tel}).fillna(0).astype(int)
...
# 3. 依照順序重新排列 (Reindex)
# 注意：使用 'reindex' 可以強制依照我們想要的順序，若資料中沒有該縣市會補 0
df_stats = df_stats.reindex(custom_order).fillna(0).astype(int)

# 4. 計算差距與佔比
df_stats['差距 (實體-通訊)'] = df_stats['青壯方案(實體)'] - df_stats['通訊諮商']
# 避免除以 0 的錯誤
total_resources = df_stats['青壯方案(實體)'] + df_stats['通訊諮商']
df_stats['通訊占比 (%)'] = (df_stats['通訊諮商'] / total_resources * 100).fillna(0).round(1)

# 5. 顯示圖表 (堆疊長條圖)
st.subheader("資源總量比較 (由北至南排序)")
st.bar_chart(df_stats[['青壯方案(實體)', '通訊諮商']], color=["#FF4B4B", "#3670CE"])

# 6. 顯示詳細表格
with st.expander("點擊查看完整詳細數據表"):
    # 為了讓表格好看，我們可以把 Index (縣市) 變成一個欄位
    st.dataframe(
        df_stats.style.highlight_max(axis=0, color='#f0f2f6'), # 高亮最大值
        use_container_width=True
    )
...
with col1:
    top_city = df_stats['通訊諮商'].idxmax()
    st.metric("通訊資源最多", f"{top_city}", f"{df_stats.loc[top_city, '通訊諮商']} 間")
with col2:
    # 找出佔比最高 (排除總數太少的，例如少於5間的以免偏差)
    valid_stats = df_stats[total_resources > 5]
    if not valid_stats.empty:
        top_ratio_city = valid_stats['通訊占比 (%)'].idxmax()
        st.metric("通訊占比最高", f"{top_ratio_city}", f"{valid_stats.loc[top_ratio_city, '通訊占比 (%)']}%")
    else:
        st.metric("通訊占比最高", "資料不足", "0%")
with col3:
    # 計算全台通訊佔比
    total_gen = df_stats['青壯方案(實體)'].sum()
    total_tel = df_stats['通訊諮商'].sum()
    avg_ratio = round(total_tel / (total_gen + total_tel) * 100, 1)
    st.metric("全台平均通訊占比", f"{avg_ratio}%", f"總差距 {total_gen - total_tel} 間")
